...
test("BeautifyConsole test", () => {
  ...
  expect(formatConsoleStr('string=%s number=%d', 'string', 1)).toStrictEqual(['string=', 'string', ' number=', 1, '']);
  expect(log.info(formatConsoleStr('string=%s number=%d', 'string', 1).join(''))).toBe(undefined);
  expect(log.info(formatConsoleStr('object=%o', { name: "chengzan" }).join(''))).toBe(undefined);
